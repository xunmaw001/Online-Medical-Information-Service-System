<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="当班诊室" prop="dangbanzhenshi">
          <el-input v-model="ruleForm.dangbanzhenshi" 
              placeholder="当班诊室" clearable  :readonly="ro.dangbanzhenshi"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="当班诊室" prop="dangbanzhenshi">
              <el-input v-model="ruleForm.dangbanzhenshi" 
                placeholder="当班诊室" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="复查日期" prop="fuchariqi">
            <el-date-picker
                format="yyyy 年 MM 月 dd 日"
                value-format="yyyy-MM-dd"
                v-model="ruleForm.fuchariqi" 
                type="date"
                :readonly="ro.fuchariqi"
                placeholder="复查日期">
            </el-date-picker> 
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.fuchariqi" label="复查日期" prop="fuchariqi">
              <el-input v-model="ruleForm.fuchariqi" 
                placeholder="复查日期" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="诊断报告" prop="zhenduanbaogao">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.zhenduanbaogao" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.zhenduanbaogao" label="诊断报告" prop="zhenduanbaogao">
                    <span v-html="ruleForm.zhenduanbaogao"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="医生嘱咐" prop="yishengzhufu">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.yishengzhufu" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.yishengzhufu" label="医生嘱咐" prop="yishengzhufu">
                    <span v-html="ruleForm.yishengzhufu"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.zhenduanbaogao = true;
          ...
          if(o=='fuchariqi'){
            this.ruleForm.fuchariqi = obj[o];
	    this.ro.fuchariqi = true;
            continue;
          }
          if(o=='yishengzhufu'){
            this.ruleForm.yishengzhufu = obj[o];
	    this.ro.yishengzhufu = true;
            continue;
          }
          if(o=='clicknum'){
	...
	this.ruleForm.zhenduanbaogao = this.ruleForm.zhenduanbaogao.replace(reg,'../../../ssm9p3y7/upload');
	this.ruleForm.yishengzhufu = this.ruleForm.yishengzhufu.replace(reg,'../../../ssm9p3y7/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `jiuzhenjieguo/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.jiuzhenjieguoCrossAddOrUpdateFlag = false;
						 this.parent.search();
